...
func main() {
	...
	assessmentComparisonService, err := comparisonuc.NewService(
		assessmentComparisonStore, assessmentSnapshotStore, assessmentCycleStore,
		postgres.NewFindingLineageRepository(pool), postgres.NewTenantTransactionRunner(pool), auditLog, clock, ids, nil, nil,
	)
	if err != nil {
		log.Error("assessment comparison service init failed", "err", err)
		os.Exit(1)
	}
	assessmentClosureReportService, err := cycleuc.NewClosureReportService(assessmentCycleStore, assessmentCycleStore, assessmentSnapshotStore, assessmentComparisonStore, auditLog)
	if err != nil {
	...
	if cfg.MetricsEnabled {
		metrics = observability.New(queue, postgres.NewPoolStatsSource(pool))
		integrationService.SetObserver(metrics)
		if !workerMetricsAddrIsLoopback(cfg.MetricsAddr) {
			log.Warn("worker metrics listener is non-loopback and unauthenticated", "addr", cfg.MetricsAddr)
		}
		go func() {
			if metricsErr := httpserver.Run(ctx, cfg.MetricsAddr, metrics.Handler(), log); metricsErr != nil && ctx.Err() == nil {
				log.Error("worker metrics listener failed", "err", metricsErr)
				stop()
			}
		}()
	}
	...
	if cfg.WorkerProfile == config.WorkerProfileIntegrations {
		runWorkerRuntime(
			...
			cfg,
			queue,
			map[string]worker.Handler{integrationuc.JobKind: integrationJobHandler{svc: integrationService}},
			integrationMaintenanceTasks,
			leaderStore,
			auditLog,
			clock,
			ids,
			cfg.ReconTimeout+time.Minute,
			log,
		)
		return
	}
...
func workerMetricsAddrIsLoopback(addr string) bool {
