...
func usageTo(w io.Writer) {
	...
	fmt.Fprintln(w, "  synapse-cli doctor [path] [--json]       # offline pre-scan readiness: toolchain, markers, and dimension coverage")
	fmt.Fprintln(w, "  synapse-cli scan <path|image-ref> [--image] [--offline] [--json] [--sarif] [--sarif-out FILE] [--mode full|vulnerabilities|licenses] [--fail-on critical|high|medium|low|info] [--min-confidence low|medium|high|very_high] [--base REF] [--include-test] [--verify-secrets] [--ignore-unfixed] [--detection-priority comprehensive|precise] [--server URL --project KEY [--branch REF] [--run-url URL] [--ci-provider NAME] [--insecure-http]]")
	fmt.Fprintln(w, "      --server   record the result on a Synapse server as the project's next analysis (token from SYNAPSE_API_TOKEN); the history, trend and managed gate in the console pick it up")
	fmt.Fprintln(w, "      --insecure-http   allow a plain-http --server that is not loopback (the token then travels in the clear)")
	fmt.Fprintln(w, "      --sarif    write a SARIF 2.1.0 report to stdout (for GitHub code-scanning upload); --fail-on still sets the exit code")
	fmt.Fprintln(w, "      --sarif-out FILE  write the SARIF report to FILE and keep the human report on stdout, so a CI log still shows what was found")
	fmt.Fprintln(w, "      --image    treat the argument as a container image reference (pulled daemonlessly, in-process) instead of a local path")
	fmt.Fprintln(w, "      --offline  no network egress: skip live OSV, every registry resolver (npm/composer/poetry/bundler/maven/gradle), KEV/EPSS, online NVD, license metadata and AI triage; detect with the local sources only – the owned advisory store, plus Grype's pre-synced DB when SYNAPSE_DETECTION_SOURCES lists it (air-gapped / fast)")
	fmt.Fprintln(w, "      --include-test  also fail the gate on findings in test/fixture/example paths (default: reported but exempt)")
	fmt.Fprintln(w, "      --verify-secrets  actively confirm each detected credential is live via one read-only provider call (opt-in; sends the secret to its issuing provider; default off)")
	fmt.Fprintln(w, "  synapse-cli publish-source [path] --server URL --project KEY --analysis ID  # stream server-inventoried source; token from SYNAPSE_API_TOKEN")
	fmt.Fprintln(w, "  synapse-cli inventory <path>             # per-language code-size inventory (files, code/comment/blank lines, functions) – no DB")
	fmt.Fprintln(w, "  synapse-cli metrics <path> [--fail-on-complexity N] [--top N]  # per-function cyclomatic+cognitive complexity (needs the synapse-ast sidecar)")
	fmt.Fprintln(w, "  synapse-cli duplication <path> [--min-tokens N] [--fail-on-duplication PCT] [--top N]  # copy-paste detection (blocks, lines, density) – no DB")
	fmt.Fprintln(w, "  synapse-cli quality <path> [--fail-on SEV] [--min-complexity N] [--include-test-smells] [--sarif]  # maintainability + reliability findings (+ duplication, + complexity via synapse-ast) – no DB")
	fmt.Fprintln(w, "      --include-test-smells  also report info-severity smells in test code (suppressed by default)")
	fmt.Fprintln(w, "  synapse-cli rating <path> [--json] [--fail-below GRADE]  # A-E health grades (security/reliability/maintainability) + technical debt – no DB")
	fmt.Fprintln(w, "  synapse-cli gate <path> [--new-code-only] [--base REF] [--gate FILE] [--rules FILE] [--coverage FILE] [--format text|markdown]  # Clean-as-You-Code quality gate")
	fmt.Fprintln(w, "  synapse-cli coverage <lcov|cobertura|jacoco file> [--fail-below PCT] [--top N]  # parse a coverage report (auto-detected)")
	fmt.Fprintln(w, "  synapse-cli rulepack verify|replay|gate ...  # signed detection RulePack verification, fixture replay, and promotion gates")
	fmt.Fprintln(w, "  synapse-cli sync-advisories <dir>        # ingest a local OSV dump into the owned advisory store (requires SYNAPSE_DB_DSN)")
	fmt.Fprintln(w, "  synapse-cli sync-advisories --remote     # fetch + ingest app ecosystems from the OSV bulk bucket (requires SYNAPSE_DB_DSN)")
	fmt.Fprintln(w, "  synapse-cli sync-advisories --remote-distros # fetch + ingest OS-package advisories (Debian/Alpine) from OSV (large; requires SYNAPSE_DB_DSN)")
	fmt.Fprintln(w, "  synapse-cli sync-advisories --remote-secdb   # fetch + ingest Alpine's own secdb, which covers current apk branches far better than the OSV mirror (small; requires SYNAPSE_DB_DSN)")
	fmt.Fprintln(w, "  synapse-cli sync-advisories --csaf <dir> # ingest a local CSAF 2.0 advisory dump (requires SYNAPSE_DB_DSN)")
	fmt.Fprintln(w, "  synapse-cli build-cvss-db <out.jsonl[.gz]> <nvd-*.json[.gz]...>  # build an OFFLINE CVSS DB from NVD JSON feeds; use it via SYNAPSE_NVD_CVSS_DB to backfill CVSS with no network/rate-limit")
}
