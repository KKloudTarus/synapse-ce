...
func run(args []string, output io.Writer) error {
	...
	service, err := comparisonuc.NewService(
		comparisons, postgres.NewAssessmentSnapshotRepository(pool), cycles, postgres.NewFindingLineageRepository(pool),
		postgres.NewTenantTransactionRunner(pool), postgres.NewAuditLog(pool), clock, ids, nil, nil,
	)
	if err != nil {
		return err
	}
