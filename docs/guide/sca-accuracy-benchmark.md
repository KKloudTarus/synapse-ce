...
Retaining the historical fixture is a conscious repository-size trade-off: the 16 immutable files total exactly 3,581,508 bytes (3.416 MiB). Most of that weight is the two canonical CycloneDX inputs and their independently frozen Oracle. This bounded cost is accepted because it preserves byte-level reducer, rendering, identity, and ratchet regression coverage without downloading mutable inputs. A content-addressed inventory freezes every path, size, and digest. The fixture contains no raw scanner database or proprietary scanner material and remains historical test evidence, never current benchmark evidence.
...
## Interpretation and database provenance

The Oracle is scanner-independent, but its package/advisory truth comes from frozen Debian and SUSE vendor evidence. The owned matcher consumes the corresponding pinned vendor OVAL database, while Grype, Trivy, and OSV-Scanner consume their own pinned database snapshots. Agreement by the owned matcher therefore measures faithful reproduction of the frozen vendor truth, including target-native EVR semantics; a difference from a comparator can also reflect database provenance, advisory coverage, or snapshot timing.

Every comparative report and PR summary must state that coupling and retain each engine's exact `database_build` and `database_digest` beside its scores. The accepted claim is scoped to agreement with the frozen vendor Oracle on the pinned targets and same SBOM bytes. It is not a market-wide claim that one engine is intrinsically more accurate. Findings outside the reviewed Oracle remain `unknown`, not false positives, so a newer or broader comparator database is not penalized for evidence the Oracle has not adjudicated.

The strict ratchet is a regression floor, not a quality endorsement. A zero measured baseline remains visible as zero and cannot be rewritten into a favorable threshold. `ratchet-baseline.json` preserves the reviewed threshold policy independently of refreshed engine-binary and database pins. Plan materialization holds the Oracle, target, SBOM, environment, configuration, and capability-kind roots stable, and rejects a ratchet that lowers a minimum, raises a maximum, enables undefined precision where the baseline forbids it, removes a governed target/engine floor, or changes its gate mode.
...
- **Accountable review**: two sanitized, immutable, repository-backed GitHub records preserve provenance without rewriting GitHub state. `GitHubReviewCapture` keeps the independent review ID/URL, login, actual state, submission timestamp, reviewed commit, and complete review body. `GitHubReviewDispositionCapture` keeps a separate PR issue comment from a distinct maintainer authority, including its ID/URL, login, creation and update timestamps, review ID, reviewed commit, final implementation commit, decision, and canonical body. An approved disposition body is exactly `decision: approved`, `review_id: <id>`, `reviewed_commit: <sha>`, and `implementation_commit: <sha>` on four lines. The captured comment must be unedited (`created_at == updated_at`). A disposition may accept the independent review's actual `COMMENTED` or `APPROVED` state, but an approved disposition cannot override `CHANGES_REQUESTED`. The accountable-review record binds both exact byte captures, both identities and timestamps, the final Oracle, and the implementation commit. Materialization copies both captures into the sanitized candidate, and publication re-hashes both. Automation cannot stand in for either human record; every final-Oracle case continues to name the independent submitted reviewer.
...
4. bind the unique sanitized GitHub review capture and distinct maintainer disposition capture to the adjudication, final Oracle, reviewed commit, and exact final implementation commit;
5. validate that the strict ratchet is monotonic against `ratchet-baseline.json`, then derive the cycle plan, repetitions, cells, dispatch count, unsupported count, and retention controls from the reviewed roots; and
...
- A non-pull-request event is a trusted full-control request only when `ENGINE_ACCURACY_TRUSTED_ENABLED` equals `true`, the resolved source ref exactly equals `ENGINE_ACCURACY_TRUSTED_REF` (default `refs/heads/main`), and the resolved source SHA exactly equals the nonempty `ENGINE_ACCURACY_TRUSTED_SHA`. Configure the trusted ref only to a protected branch that blocks force-pushes and requires the repository's review/check policy; prefer the default branch. Set the exact SHA only for the authorized run and clear the enablement and SHA variables after collection. A later push to the same ref therefore cannot inherit trusted execution. Disabled, SHA-mismatched, or ref-mismatched events are explicit non-evaluation modes in the aggregate.
- The trusted runner proves both ephemeral registration and the hardened execution boundary before capture. `SCA_ACCURACY_ACTIONS_RUNNER_ROOT` must be an absolute runner root whose `.runner` file is readable, regular, non-symlinked, and records `"ephemeral": true`; preflight hashes that file and retains the digest plus `runner_ephemeral: true` in the audit-safe capture status. The wrapper also proves a real direct-cgroup plus Bubblewrap no-op child. It captures the delegated **service** cgroup root before the benchmark process enters `synapse-manager` and supplies it as `SCA_ACCURACY_DELEGATED_CGROUP_ROOT`; the runner rejects missing, non-writable, non-service-root, memory/pids-disabled, or non-allocatable roots. It receives this root, pinned Bubblewrap, Docker images, both review captures, pre-frozen SBOM provenance, and retained-input bootstrap as an explicit external ephemeral-runner contract; it does not use a host `/proc/self/status` seccomp assertion.
- Ephemeral configuration is necessary but not sufficient proof of disposal. After the job, the external cleanup controller must record that the local `.runner`, `.credentials`, runner root, and service unit no longer exist; the GitHub runner registration is absent; protected raw retention and Docker state are removed; and the disposable AWS host and owning stack have been deleted. Final PR evidence must identify that cleanup record. The accepted sanitized Actions artifact is the sole retained run artifact and expires under the reviewed 90-day policy.
