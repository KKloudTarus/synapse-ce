...
Ubuntu, Debian, Oracle Linux, AlmaLinux, openSUSE, and SUSE Linux Enterprise OVAL feeds plus Amazon Linux updateinfo, Rocky Linux Apollo OSV, and apk secdb (Alpine, Wolfi, Chainguard) so that detection does not depend on one provider. A freshness check warns
