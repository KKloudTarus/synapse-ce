...
| `SYNAPSE_GOBIN_BINDING_REPORT_DIR` | unset | Benchmark-only directory containing sanitized `api.json` and `worker.json` binding reports. The `synapse-reachability-cycle current-go-binary-scorecard` command requires both files and fails when either is missing or malformed. This setting does not change API or worker scanning. |
