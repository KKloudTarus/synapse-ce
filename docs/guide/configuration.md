...
| `SYNAPSE_SECRET_VERIFY_ENABLED` | `false` | Opt-in active secret verification (D6.3): for a detected credential whose provider is supported, make one minimal read-only API call to confirm it is live, and stamp the finding verified/unverified/unknown. SECURITY-SENSITIVE and OFF by default: it sends the raw leaked secret over the network to the provider's PUBLIC host (`api.github.com`, `gitlab.com`, `api.openai.com`), so enable it ONLY against credentials you are authorized to test. A credential for a self-managed instance (GitHub Enterprise, self-hosted GitLab) shares the public token shape and is still sent to the public host, where it yields `unknown` (never a false "not live"); avoid this feature if that disclosure is unacceptable. The request uses an SSRF-hardened, rate-limited client (no proxy, no redirects), the secret is never logged, sealed, or written to the report, and at most 100 distinct credentials are verified per scan. A verified live credential is raised to `very_high` confidence (above the high base of the fixed-prefix rules); an unverified or unknown verdict never suppresses a finding. |
...
| `SYNAPSE_OWNERSHIP_MODE` | `off` | Finding ownership API rollout: `off`, `observe`, or `enforce`; invalid values stop startup. Enabled modes require PostgreSQL and allow team administration and manual triage. Production routing is pending: preview, reroute, and release return `503` and `routing_available` is false. Memory mode reports `postgres_required`. See [ownership API](../ownership-api.md). |
