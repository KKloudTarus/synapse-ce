...
`internal/infrastructure/reachbench/testdata/go_binary_versioned/` contains source for four Linux/amd64 binaries. `go.mod` and `go.sum` pin `golang.org/x/net@v0.59.0` and its indirect `golang.org/x/text@v0.42.0` dependency. `direct.go.txt` calls `golang.org/x/net/idna.ToASCII` from `main`; `retained.go.txt` keeps a function that calls the same symbol in the binary but never invokes it from `main`; `retained_called.go.txt` invokes that retained function; and `indirect.go.txt` conditionally passes the call through a function value when invoked with the `invoke` argument. Build with Go 1.27.0, `CGO_ENABLED=0`, `GOOS=linux`, `GOARCH=amd64`, `GOTOOLCHAIN=local`, and `go build -trimpath`. The Linux package test verifies both argument paths and exact IDNA-derived output, then derives the observed result through the production capture adapter.

The proposal manifest has SHA-256 `635fc5007563150854e4a68096e11b5e3f22f7d63e9f11ac9a967b5f47f3a5e6`. Its `files_sha256` map pins the six input files. This digest identifies the proposal bytes only; it is not a trusted full-contract fingerprint. Source SHA and toolchain identity must accompany any review run. The package test runs on the repository's Go toolchain; a Linux Docker run can use `golang:1.27.0` with the repository mounted at `/work`.
...
The proposal records five non-authoritative, pending-review oracle candidates: the three direct controls, the function-value call, and an empty-root unavailable control. Its separate `candidate_observations` table pins each candidate's exact query identity, source/materialization, observed outcome and coverage, and non-suppressing judgment state. The package test compares that table directly to production capture. The two version-identity diagnostics remain adversarial identity checks; they are not successor-oracle cases. An independent reviewer must confirm the source truth and expected coverage/outcome before any candidate is used as an oracle.

The current production observation for both the retained-but-uncalled and function-value cases is `no_analysis` with unavailable coverage. That result is recorded as analyzer behavior; it does not change the independently stated source truth or the pending oracle mapping.
