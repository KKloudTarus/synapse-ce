...
- **Opt-in active secret verification (`SYNAPSE_SECRET_VERIFY_ENABLED` / `--verify-secrets`, off by default).** When enabled, a detected credential whose provider is supported (GitHub, GitLab, OpenAI) is checked with one minimal read-only API call to that provider's public host, and the finding is stamped verified/unverified/unknown. A verified live credential is raised to `very_high` confidence (above the high base of the fixed-prefix secret rules), so a confirmed-live leak sorts to the top; an unverified or unknown verdict never suppresses a finding, because a committed-then-rotated secret is still a leak to remove. The feature is security-sensitive and off by default (and ignored under the CLI `--offline` flag): it sends the raw leaked secret over the network to the provider's public host, so the raw value is confined to the scanner and the single provider call, never logged, sealed, or written to the report; the outbound request uses an SSRF-hardened, rate-limited client (no proxy, no redirects; `SYNAPSE_SECRET_VERIFY_RPS`, default 5), and at most 100 distinct credentials are verified per scan. A public-host rejection of a host-ambiguous token (a GitHub Enterprise or self-managed GitLab credential) maps to `unknown`, never a false "not live". Verification runs only inside a scan the engagement's authorization window already gated. Providers that need more than a single bearer token (AWS, which needs the access-key + secret-key pair; Vault, which needs a configured address) return `unknown` rather than a fabricated verdict, and are a documented follow-up. Implements EPIC #860 D6.3.
...
- **Finding ownership gains a PostgreSQL-backed administration and triage API.** Optional `SYNAPSE_OWNERSHIP_MODE=observe|enforce` enables tenant-scoped teams, membership and owner mappings, immutable CODEOWNERS imports and policy versions, an inbox with existing SLA filters, assignment/claim/transfer, idempotent bulk results and decision history. Legacy assignee writes share the assignment/audit transaction and retain optimistic version behavior. Transfer preserves an eligible assignee or requires an explicit replacement/clear. The feature defaults off; memory mode reports unavailable. Production source capture and routing workers remain pending, so preview, reroute and release to automatic routing explicitly return `503` at this checkpoint. See [the ownership API guide](docs/ownership-api.md).
