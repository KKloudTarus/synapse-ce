...
- **Text rule pack.** Added parser-free security and repository-hygiene checks for Trojan Source controls, invisible Unicode, UTF-8 BOMs, mixed line endings, trailing whitespace, oversized files, missing final newlines, and overlong lines. Full oversized text is scanned incrementally with bounded memory; finding caps prioritize security and report truncation explicitly.
