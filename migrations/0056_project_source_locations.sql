...
ALTER TABLE project_issues
    ...
    ADD CONSTRAINT project_issues_source_range CHECK (
        ...
        (source_file IS NOT NULL AND start_line IS NOT NULL AND end_line IS NOT NULL AND
         start_line >= 1 AND end_line >= start_line AND
         ((start_column IS NULL AND end_column IS NULL) OR
          (start_column IS NOT NULL AND end_column IS NOT NULL AND start_column >= 0 AND end_column >= 0)))
    );
...
WHERE file <> ''
  ...
  AND (regexp_match(location, '^(.+):([1-9][0-9]*)$'))[1] = file
  AND file !~ '(^|/)\.\.(/|$)'
  AND file !~ '(^|/)\./';
