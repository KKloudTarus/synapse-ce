...
MEASUREMENT_RE = re.compile(
    ...
    r".*?\bpeak_mem=(?P<peak>[1-9][0-9]*)\b.*?\blatency_p50=(?P<p50>[^\s]+)"
    r"\s+latency_p95=(?P<p95>[^\s]+)\s+dataset=(?P<dataset>sha256:[0-9a-f]{64})\b"
)
...
def load_baselines(directory, require_release_sha=True, require_throughput=True):
...
def verify(candidate_baselines_dir, control_baselines_dir, candidate_dir, control_dir, candidate_sha):
    ...
    control = load_baselines(control_baselines_dir, require_release_sha=False, require_throughput=False)
    ...
    for name, (test_name, log_name) in BENCHMARKS.items():
        ...
        candidate_measurement = read_measurement(Path(candidate_dir) / log_name, test_name)
        control_measurement = read_measurement(Path(control_dir) / log_name, test_name)
        for label, measurement, expected_release in (("candidate", candidate_measurement, candidate_sha), ("control", control_measurement, control_source)):
            if measurement["release"] != expected_release:
                raise ValueError(f"{label} measurement release mismatch for {name}")
            if measurement["dataset"] != candidate_baseline["dataset_digest"]:
                raise ValueError(f"{label} measurement dataset mismatch for {name}")
            if measurement["samples"] != candidate_baseline["samples"]:
                raise ValueError(f"{label} measurement sampling policy mismatch for {name}")
        ceiling = max(control_measurement["p50_millis"] * 1.5, control_measurement["p50_millis"] + 5.0)
        if candidate_measurement["p50_millis"] > ceiling:
            raise ValueError(
                f"candidate p50 regression for {name}: {candidate_measurement['p50_millis']:.3f}ms exceeds {ceiling:.3f}ms "
                f"(control {control_measurement['p50_millis']:.3f}ms)"
            )
        rows.append({
            "baseline": name, "test": test_name, "control_p50_millis": control_measurement["p50_millis"],
            "candidate_p50_millis": candidate_measurement["p50_millis"], "p50_ceiling_millis": ceiling,
            "control_p95_millis": control_measurement["p95_millis"], "candidate_p95_millis": candidate_measurement["p95_millis"],
            "control_peak_memory_bytes": control_measurement["peak"], "candidate_peak_memory_bytes": candidate_measurement["peak"],
        })
    return {"schema": "synapse-performance-latency-comparison-v1", "control_release_digest": control_source, "candidate_release_digest": candidate_sha, "comparisons": rows}
