...
func TestBuildSandboxedSpecAndParse(t *testing.T) {
	wire := `{"protocol_version":"v1.2.0","entrypoints":["m.main"],"edges":[{"caller":"m.main","callees":["m.run"]}]}`
...
func TestNewDefaultsBinaryName(t *testing.T) {
	...
	fr := &fakeRunner{result: ports.ToolResult{Stdout: []byte(`{"protocol_version":"v1.2.0"}`)}}
