...
// mixed-content asset directories (where a non-asset file must still be scanned).
func TestSkipsVendoredMinifiedAndGeneratedFiles(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "js under assets", rel: "app/assets/javascripts/vendor.js", content: "var h = CryptoJS.MD5(x);\n", want: false},
		{name: "css under public", rel: "public/theme.css", content: "/* CryptoJS.MD5 */\nbody { color: red }\n", want: false},
