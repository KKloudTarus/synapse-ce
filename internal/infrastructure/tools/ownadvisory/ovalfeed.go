...
// OVALDirFeed is an AdvisoryFeed over a local directory of deb-family OVAL files (Canonical Ubuntu or Debian)
// (Ubuntu com.ubuntu.<codename>.cve.oval.xml or Debian oval-definitions-<release>.xml, plain or .bz2): the offline ingestion path for vendor OS-package
// advisories. Each file is one release's whole CVE set, parsed via ParseOVAL over the same hardened
...
// Each walks the directory, parses every OVAL file via ParseOVAL (Ubuntu or Debian, auto-detected), and invokes fn for each advisory
...
func (f *OVALDirFeed) Each(ctx context.Context, fn func(a advisory.Advisory) error) (int, error) {
	inert := 0
	fileSkipped, err := walkAdvisoryFiles(ctx, f.dir, hasOVALSuffix, maxOVALFileBytes, ParseOVAL, func(adv advisory.Advisory) error {
