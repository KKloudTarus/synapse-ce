...
func cExtendedRules() []rule.Rule {
	...
	maint := []cRuleSpec{
		cRule("long-function-body", "Function body exceeds statement count", "CWE-1120", "void build(void) { step_a(); step_b(); }", "void build(void) { /* >50 statements */ }", "Refactor large functions into focused helper functions.", rule.TypeCodeSmell, rule.QualityMaintainability, shared.SeverityLow),
		...
		cRule("single-letter-identifier", "Single-letter variable name in wide scope", "CWE-1120", "int buffer_index = 0;", "int i = 0; /* used across 50 lines */", "Use meaningful variable names for identifiers used across wide scopes.", rule.TypeCodeSmell, rule.QualityMaintainability, shared.SeverityLow),
