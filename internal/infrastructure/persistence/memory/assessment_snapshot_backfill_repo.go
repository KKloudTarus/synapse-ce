...
func (repository *AssessmentSnapshotBackfillRepository) SaveAssessmentSnapshotBackfillItem(_ context.Context, item ports.AssessmentSnapshotBackfillItem) (bool, error) {
	...
	if run.State != ports.AssessmentSnapshotBackfillRunning {
		return false, fmt.Errorf("%w: assessment snapshot backfill run is not active", shared.ErrConflict)
	}
...
func (repository *AssessmentSnapshotBackfillRepository) AdvanceAssessmentSnapshotBackfillRun(_ context.Context, tenantID, runID shared.ID, leaseOwner string, checkpoint shared.ID, now time.Time, leaseDuration time.Duration) (ports.AssessmentSnapshotBackfillRun, error) {
	repository.mu.Lock()
	defer repository.mu.Unlock()
	tenantID = shared.TenantOrDefault(tenantID)
	run, ok := repository.runs[tenantID][runID]
	...
	if run.State != ports.AssessmentSnapshotBackfillRunning || run.LeaseOwner != strings.TrimSpace(leaseOwner) || checkpoint < run.CheckpointAssessment || leaseDuration <= 0 {
...
func (repository *AssessmentSnapshotBackfillRepository) FinishAssessmentSnapshotBackfillRun(_ context.Context, tenantID, runID shared.ID, leaseOwner string, state ports.AssessmentSnapshotBackfillState, now time.Time) (ports.AssessmentSnapshotBackfillRun, error) {
	...
	if run.State != ports.AssessmentSnapshotBackfillRunning || run.LeaseOwner != strings.TrimSpace(leaseOwner) {
