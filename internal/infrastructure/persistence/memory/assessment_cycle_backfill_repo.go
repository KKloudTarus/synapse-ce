...
func (repository *AssessmentCycleBackfillRepository) SaveAssessmentCycleBackfillItem(_ context.Context, item ports.AssessmentCycleBackfillItem) (bool, error) {
	...
	if run.State != ports.AssessmentCycleBackfillRunning {
		return false, fmt.Errorf("%w: assessment cycle backfill run is not active", shared.ErrConflict)
	}
...
func (repository *AssessmentCycleBackfillRepository) AdvanceAssessmentCycleBackfillRun(_ context.Context, tenantID, runID shared.ID, leaseOwner string, checkpoint shared.ID, now time.Time, leaseDuration time.Duration) (ports.AssessmentCycleBackfillRun, error) {
	repository.mu.Lock()
	defer repository.mu.Unlock()
	...
	if run.State != ports.AssessmentCycleBackfillRunning || run.LeaseOwner != strings.TrimSpace(leaseOwner) || checkpoint < run.CheckpointAssessment || leaseDuration <= 0 {
...
func (repository *AssessmentCycleBackfillRepository) FinishAssessmentCycleBackfillRun(_ context.Context, tenantID, runID shared.ID, leaseOwner string, state ports.AssessmentCycleBackfillState, now time.Time) (ports.AssessmentCycleBackfillRun, error) {
	...
	if run.State != ports.AssessmentCycleBackfillRunning || run.LeaseOwner != strings.TrimSpace(leaseOwner) {
