...
func (s *ScanRunStore) SealScanRun(_ context.Context, tenantID shared.ID, runID string, terminalStatus scanrun.TerminalStatus, lanes []scanrun.Lane, manifestSchemaVersion int, manifestHash string, sealedAt time.Time) error {
	if tenantID.IsZero() || runID == "" {
	...
	// Idempotent duplicate seal check
	if existing.IsSealed() {
...
func toPortsScanRun(r scanrun.ScanRun) ports.ScanRun {
	...
	return ports.ScanRun{
		...
		Manifest:     manifest,
		FindingKeys:  r.LegacyFindingKeys,
	}
}
...
func cloneScanRun(r scanrun.ScanRun) scanrun.ScanRun {
	...
	if len(r.Lanes) > 0 {
		...
		for i, l := range r.Lanes {
			...
			if l.SealedAt != nil {
				t := *l.SealedAt
				out.Lanes[i].SealedAt = &t
			}
			if len(l.AuthoritativeFindingKinds) > 0 {
				out.Lanes[i].AuthoritativeFindingKinds = append([]string(nil), l.AuthoritativeFindingKinds...)
			}
			if len(l.IncludedScope) > 0 {
				out.Lanes[i].IncludedScope = append([]string(nil), l.IncludedScope...)
			}
			if len(l.ExcludedScope) > 0 {
				out.Lanes[i].ExcludedScope = append([]string(nil), l.ExcludedScope...)
			}
			if len(l.Versions) > 0 {
				out.Lanes[i].Versions = append([]scanrun.LaneVersion(nil), l.Versions...)
			}
			if len(l.Stages) > 0 {
				out.Lanes[i].Stages = append([]scanrun.LaneStage(nil), l.Stages...)
			}
		}
	}
	return out
