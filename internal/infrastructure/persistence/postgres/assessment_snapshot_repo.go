...
const assessmentSnapshotColumns = `tenant_id,id,cycle_id,assessment_id,snapshot_number,lifecycle,provenance,boundary_kind,
...
func (repository *AssessmentSnapshotRepository) CreateFinalizedCAS(ctx context.Context, snapshot *assessmentsnapshot.Snapshot, expectedDefaultVersion int64) (*assessmentsnapshot.Snapshot, bool, error) {
	...
	err := WithTenant(ctx, repository.pool, tenantID.String(), func(tx pgx.Tx) error {
		...
		if err := insertAndFinalizeAssessmentSnapshot(ctx, tx, tenantID, snapshot, snapshotNumber); err != nil {
			return err
		}
		...
		stored = cloneFinalizedSnapshot(snapshot, snapshotNumber)
		created = true
		return nil
	})
...
func (repository *AssessmentSnapshotRepository) CreateLegacyProjection(ctx context.Context, snapshot *assessmentsnapshot.Snapshot) (*assessmentsnapshot.Snapshot, bool, error) {
	...
	err := WithTenant(ctx, repository.pool, tenantID.String(), func(tx pgx.Tx) error {
		...
		stored, created = cloneFinalizedSnapshot(snapshot, snapshotNumber), true
		return nil
	})
	return stored, created, err
}

func insertAndFinalizeAssessmentSnapshot(ctx context.Context, tx pgx.Tx, tenantID shared.ID, snapshot *assessmentsnapshot.Snapshot, snapshotNumber int) error {
	_, err := tx.Exec(ctx, `INSERT INTO assessment_snapshots(
		tenant_id,id,cycle_id,assessment_id,snapshot_number,lifecycle,provenance,boundary_kind,business_asset_id,project_id,
		schema_version,content_hash,request_key,request_hash,created_at,created_by)
		VALUES($1,$2,$3,$4,$5,'building',$6,$7,NULLIF($8,''),NULLIF($9,''),$10,$11,$12,$13,$14,$15)`,
		tenantID.String(), snapshot.ID.String(), snapshot.CycleID.String(), snapshot.AssessmentID.String(), snapshotNumber,
...
func scanAssessmentSnapshot(row rowScanner) (*assessmentsnapshot.Snapshot, error) {
	...
	if err := row.Scan(&tenantID, &id, &cycleID, &assessmentID, &snapshot.SnapshotNumber, &lifecycle, &provenance, &boundaryKind,
...
func cloneFinalizedSnapshot(snapshot *assessmentsnapshot.Snapshot, snapshotNumber int) *assessmentsnapshot.Snapshot {
