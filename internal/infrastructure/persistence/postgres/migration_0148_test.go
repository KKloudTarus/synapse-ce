...
func TestMigration0148DefinesAppendOnlyTenantSafeReviewArtifacts(t *testing.T) {
	data, err := migrations.FS.ReadFile("0148_assessment_relationship_candidates.sql")
	if err != nil {
		t.Fatal(err)
	}
	sql := string(data)
...
func TestMigration0148UpgradePaths(t *testing.T) {
	for _, from := range []int64{137, 138, 147} {
...
func TestMigration0148RollbackGuard(t *testing.T) {
	...
	tenantID, candidate := seedPostgresRelationshipSubjects(t, context.Background(), pool, "migration-0147")
	...
	pool.Close()
	if err := goose.DownTo(db, ".", 147); err == nil || !strings.Contains(err.Error(), "cannot roll back assessment relationship review while artifacts exist") {
