...
func TestMigration0147UpgradePaths(t *testing.T) {
	for _, from := range []int64{137, 138, 146} {
...
func TestMigration0147ClosureManifestGuards(t *testing.T) {
	ctx := context.Background()
	...
	if err := goose.DownTo(db, ".", 146); err == nil || !strings.Contains(err.Error(), "cannot roll back assessment closure manifests") {
