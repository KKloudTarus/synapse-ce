...
func TestPostgresScanRunStore_NativeSealedProvenance(t *testing.T) {
	...
	if err := store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantID, RunID: runID, TerminalStatus: scanrun.StatusSucceeded, ManifestSchemaVersion: 1, SealedAt: now}); !errors.Is(err, shared.ErrValidation) {
	...
	if err := store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantID, RunID: runID, TerminalStatus: scanrun.StatusSucceeded, Lanes: []scanrun.Lane{lane}, ManifestSchemaVersion: 1, ManifestHash: manifestHash, SealedAt: sealedAt}); err != nil {
	...
	err = store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantID, RunID: runID, TerminalStatus: scanrun.StatusFailed, Lanes: []scanrun.Lane{lane}, ManifestSchemaVersion: 1, ManifestHash: manifestHash, SealedAt: sealedAt})
...
func TestPostgresScanRunStore_ConcurrentSealing(t *testing.T) {
	...
	lane1Hash, err := scanrun.ComputeManifestHash(lane1)
	if err != nil {
		t.Fatal(err)
	}
	lane1.ManifestHash = lane1Hash
	runHash1, err := scanrun.ComputeRunManifestHash([]scanrun.Lane{lane1})
	if err != nil {
		t.Fatal(err)
	}
	lane2Hash, err := scanrun.ComputeManifestHash(lane2)
	if err != nil {
		t.Fatal(err)
	}
	lane2.ManifestHash = lane2Hash
	runHash2, err := scanrun.ComputeRunManifestHash([]scanrun.Lane{lane2})
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		defer wg.Done()
		results[0] = store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantID, RunID: runID, TerminalStatus: scanrun.StatusSucceeded, Lanes: []scanrun.Lane{lane1}, ManifestSchemaVersion: 1, ManifestHash: runHash1, SealedAt: now})
	}()
	go func() {
		defer wg.Done()
		results[1] = store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantID, RunID: runID, TerminalStatus: scanrun.StatusFailed, Lanes: []scanrun.Lane{lane2}, ManifestSchemaVersion: 1, ManifestHash: runHash2, SealedAt: now})
	}()
...
func TestPostgresScanRunStore_SealValidationParity(t *testing.T) {
	...
	err := store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantID, RunID: runID, TerminalStatus: scanrun.StatusBuilding, Lanes: []scanrun.Lane{lane}, ManifestSchemaVersion: 1, ManifestHash: "hash1", SealedAt: now})
	if !errors.Is(err, shared.ErrValidation) {
		t.Fatalf("expected ErrValidation when sealing with StatusBuilding, got %v", err)
	}

	// 2. Zero sealedAt timestamp -> ErrValidation
	err = store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantID, RunID: runID, TerminalStatus: scanrun.StatusSucceeded, Lanes: []scanrun.Lane{lane}, ManifestSchemaVersion: 1, ManifestHash: "hash1"})
	if !errors.Is(err, shared.ErrValidation) {
...
func TestPostgresScanRunStore_ResealIdempotencyAndReproducibility(t *testing.T) {
	...
	if err := store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantID, RunID: runID, TerminalStatus: scanrun.StatusSucceeded, Lanes: []scanrun.Lane{laneA, laneB}, ManifestSchemaVersion: scanrun.CurrentManifestSchemaVersion, ManifestHash: aggregateHash, SealedAt: now}); err != nil {
		t.Fatalf("first seal: %v", err)
	}

	// 2. Second seal using [laneB, laneA] reverse order -> MUST be idempotent success, NOT ErrConflict
	if err := store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantID, RunID: runID, TerminalStatus: scanrun.StatusSucceeded, Lanes: []scanrun.Lane{laneB, laneA}, ManifestSchemaVersion: scanrun.CurrentManifestSchemaVersion, ManifestHash: aggregateHash, SealedAt: now}); err != nil {
...
func TestPostgresScanRunStore_DirectSQLTriggerDefense(t *testing.T) {
	...
	if err := store.SealScanRun(ctx, ports.SealScanRunCommand{TenantID: tenantA, RunID: runID, TerminalStatus: scanrun.StatusSucceeded, Lanes: []scanrun.Lane{lane}, ManifestSchemaVersion: 1, ManifestHash: runHash, SealedAt: now}); err != nil {
		t.Fatalf("seal scan run: %v", err)
	}
