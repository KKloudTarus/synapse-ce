...
func TestPostgresFindingLineageBackfillConcurrentLeaseAndCompositeOwnership(t *testing.T) {
	...
	requests := []ports.FindingLineageBackfillAcquireRequest{
		{Run: ports.FindingLineageBackfillRun{TenantID: tenantID, ID: "lease-run-a", SchemaVersion: 1, BatchSize: 500, SnapshotAt: now, State: ports.FindingLineageBackfillRunning, LeaseOwner: "owner-a", CreatedBy: "operator", CreatedAt: now, UpdatedAt: now}, LeaseDuration: time.Minute},
		{Run: ports.FindingLineageBackfillRun{TenantID: tenantID, ID: "lease-run-b", SchemaVersion: 1, BatchSize: 500, SnapshotAt: now, State: ports.FindingLineageBackfillRunning, LeaseOwner: "owner-b", CreatedBy: "operator", CreatedAt: now, UpdatedAt: now}, LeaseDuration: time.Minute},
	}
	...
	resumed, wasResumed, err := repository.AcquireFindingLineageBackfillRun(ctx, ports.FindingLineageBackfillAcquireRequest{
		Run: ports.FindingLineageBackfillRun{
			...
			SnapshotAt: active.SnapshotAt, State: ports.FindingLineageBackfillRunning, LeaseOwner: "owner-resumed", CreatedBy: "operator", CreatedAt: now.Add(2 * time.Minute), UpdatedAt: now.Add(2 * time.Minute),
		},
	...
	if _, err := repository.SaveFindingLineageBackfillItem(ctx, item); err == nil {
