...
const findingLineageBackfillRunColumns = `tenant_id,id,schema_version,dry_run,batch_size,producer_filters,snapshot_at,checkpoint_finding_id,state,lease_owner,lease_expires_at,processed_count,observation_created_count,provisional_candidate_count,skipped_count,created_by,created_at,updated_at,completed_at`
...
func (repository *FindingLineageBackfillRepository) AcquireFindingLineageBackfillRun(ctx context.Context, request ports.FindingLineageBackfillAcquireRequest) (run ports.FindingLineageBackfillRun, resumed bool, err error) {
	...
	err = WithTenant(ctx, repository.pool, request.Run.TenantID.String(), func(tx pgx.Tx) error {
		...
		if scanErr == nil {
			...
			if existing.DryRun != request.Run.DryRun || existing.BatchSize != request.Run.BatchSize || strings.Join(existing.ProducerFilters, "\x00") != strings.Join(request.Run.ProducerFilters, "\x00") {
				return fmt.Errorf("%w: resumed finding lineage backfill options changed", shared.ErrConflict)
			}
			updated, err := scanFindingLineageBackfillRun(tx.QueryRow(ctx, `UPDATE finding_lineage_backfill_runs SET lease_owner=$3,lease_expires_at=$4,updated_at=$5 WHERE tenant_id=$1 AND id=$2 AND state='running' RETURNING `+findingLineageBackfillRunColumns,
		...
		created, err := scanFindingLineageBackfillRun(tx.QueryRow(ctx, `INSERT INTO finding_lineage_backfill_runs (`+findingLineageBackfillRunColumns+`) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'running',$9,$10,0,0,0,0,$11,$12,$12,NULL) RETURNING `+findingLineageBackfillRunColumns,
			...
			request.Run.SnapshotAt, request.InitialCheckpoint.String(), request.Run.LeaseOwner, request.Run.CreatedAt.Add(request.LeaseDuration), request.Run.CreatedBy, request.Run.CreatedAt))
		if err != nil {
...
func (repository *FindingLineageBackfillRepository) SaveFindingLineageBackfillItem(ctx context.Context, item ports.FindingLineageBackfillItem) (created bool, err error) {
...
func (repository *FindingLineageBackfillRepository) AdvanceFindingLineageBackfillRun(ctx context.Context, tenantID, runID shared.ID, leaseOwner string, checkpoint shared.ID, now time.Time, leaseDuration time.Duration) (run ports.FindingLineageBackfillRun, err error) {
	tenantID, leaseOwner, now = shared.TenantOrDefault(tenantID), strings.TrimSpace(leaseOwner), now.UTC()
	if checkpoint.IsZero() || leaseDuration <= 0 {
		return ports.FindingLineageBackfillRun{}, fmt.Errorf("%w: finding lineage backfill checkpoint is invalid", shared.ErrValidation)
	}
	err = WithTenant(ctx, repository.pool, tenantID.String(), func(tx pgx.Tx) error {
		updated, err := scanFindingLineageBackfillRun(tx.QueryRow(ctx, `UPDATE finding_lineage_backfill_runs AS run SET checkpoint_finding_id=$4,updated_at=$5,lease_expires_at=$6,
			...
			WHERE run.tenant_id=$1 AND run.id=$2 AND run.state='running' AND run.lease_owner=$3 AND run.checkpoint_finding_id<=$4 RETURNING `+findingLineageBackfillRunColumns,
			tenantID.String(), runID.String(), leaseOwner, checkpoint.String(), now, now.Add(leaseDuration)))
		if errors.Is(err, pgx.ErrNoRows) {
...
func (repository *FindingLineageBackfillRepository) FinishFindingLineageBackfillRun(ctx context.Context, tenantID, runID shared.ID, leaseOwner string, state ports.FindingLineageBackfillState, now time.Time) (run ports.FindingLineageBackfillRun, err error) {
	...
	tenantID, leaseOwner, now = shared.TenantOrDefault(tenantID), strings.TrimSpace(leaseOwner), now.UTC()
	err = WithTenant(ctx, repository.pool, tenantID.String(), func(tx pgx.Tx) error {
		finished, err := scanFindingLineageBackfillRun(tx.QueryRow(ctx, `UPDATE finding_lineage_backfill_runs AS run SET state=$4,lease_owner='',lease_expires_at=NULL,updated_at=$5,completed_at=$5,
			processed_count=(SELECT count(*) FROM finding_lineage_backfill_items item WHERE item.tenant_id=run.tenant_id AND item.run_id=run.id),
			observation_created_count=(SELECT count(*) FROM finding_lineage_backfill_items item WHERE item.tenant_id=run.tenant_id AND item.run_id=run.id AND item.outcome='observation_created'),
			provisional_candidate_count=(SELECT count(*) FROM finding_lineage_backfill_items item WHERE item.tenant_id=run.tenant_id AND item.run_id=run.id AND item.outcome='provisional_candidate_created'),
			skipped_count=(SELECT count(*) FROM finding_lineage_backfill_items item WHERE item.tenant_id=run.tenant_id AND item.run_id=run.id AND item.outcome='skipped')
			WHERE run.tenant_id=$1 AND run.id=$2 AND run.state='running' AND run.lease_owner=$3 RETURNING `+findingLineageBackfillRunColumns,
...
func scanFindingLineageBackfillRun(row rowScanner) (ports.FindingLineageBackfillRun, error) {
	...
	if err := row.Scan(&run.TenantID, &run.ID, &run.SchemaVersion, &run.DryRun, &run.BatchSize, &run.ProducerFilters, &run.SnapshotAt, &run.CheckpointFinding, &state, &run.LeaseOwner, &leaseExpires,
