...
func TestPostgresAssessmentComparisonServiceParity(t *testing.T) {
	...
	service, err := comparisonuc.NewService(
		...
		NewTenantTransactionRunner(pool), postgresLineageAudit{}, postgresLineageClock{now: now.Add(2 * time.Second)}, &postgresLineageIDs{prefix: "comparison-service-id"}, nil, nil,
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestMigration0143RollbackGuard(t *testing.T) {
	...
	pool.Close()
	if err := goose.DownTo(db, ".", 142); err == nil || !strings.Contains(err.Error(), "cannot roll back assessment comparisons") {
