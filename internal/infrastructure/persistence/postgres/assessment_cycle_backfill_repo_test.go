...
func TestPostgresAssessmentCycleBackfillLeaseAndCompositeFK(t *testing.T) {
	...
	request := ports.AssessmentCycleBackfillAcquireRequest{Run: ports.AssessmentCycleBackfillRun{
		...
		LeaseOwner: "owner-a", LeaseExpiresAt: now.Add(time.Minute), CreatedBy: "operator", CreatedAt: now, UpdatedAt: now,
	...
	if _, err := repository.SaveAssessmentCycleBackfillItem(ctx, ports.AssessmentCycleBackfillItem{
		TenantID: "lease-a", RunID: "run-a", AssessmentID: "assessment-b", SchemaVersion: 1, IdempotencyKey: "source-v1", Outcome: "failed", ReasonCode: "source_not_found", RepairGuidance: "repair", ProcessedAt: now,
	}); err == nil {
