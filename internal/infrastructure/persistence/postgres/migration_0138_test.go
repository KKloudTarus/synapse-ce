...
func TestMigration0138ProductionOwnerPath(t *testing.T) {
	...
	const tenantID = "tenant-0138"
	const engagementID = "engagement-0138"
	const runID = "legacy-run-0138"
	...
	const overlapRunID = "legacy-overlap-run-0138"
	if _, err := pool.Exec(ctx, `
		INSERT INTO scan_runs(id,engagement_id,created_at,manifest,finding_keys)
		VALUES($1,$2,$3,'{}'::jsonb,'[]'::jsonb)
	`, overlapRunID, engagementID, time.Now().UTC()); err != nil {
		t.Fatalf("pre-0138 writer failed during overlap: %v", err)
	}
	...
	if err := pool.QueryRow(ctx, `SELECT tenant_id FROM scan_runs WHERE id=$1`, overlapRunID).Scan(&overlapTenant); err != nil {
		t.Fatalf("pre-0138 reader failed during overlap: %v", err)
	}
...
func isolatedMigrationOwnerDSN(t *testing.T) string {
	...
	databaseName := "synapse_migration_0138_" + suffix
