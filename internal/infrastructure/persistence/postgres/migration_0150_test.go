...
func TestMigration0150VisibleProjectBoundaryAndRollbackGuard(t *testing.T) {
	...
	if err := goose.DownTo(db, ".", 149); err == nil || !strings.Contains(err.Error(), "associations exist") {
		t.Fatalf("unsafe rollback accepted: %v", err)
	}
}

func TestMigrations0150And0151EmptyRoundTrip(t *testing.T) {
