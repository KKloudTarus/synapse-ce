...
func TestMigration0150DefinesAppendOnlyTenantSafeReviewArtifacts(t *testing.T) {
	data, err := migrations.FS.ReadFile("0150_assessment_relationship_candidates.sql")
	if err != nil {
		t.Fatal(err)
	}
	sql := string(data)
...
func TestMigration0150EmptyRoundTrip(t *testing.T) {
	...
	if err := goose.DownTo(db, ".", 149); err != nil {
		t.Fatalf("rollback to 0149: %v", err)
	}
}

func TestMigration0150UpgradePaths(t *testing.T) {
	for _, from := range []int64{128, 140, 149} {
...
func TestMigration0150RollbackGuard(t *testing.T) {
	...
	tenantID, candidate := seedPostgresRelationshipSubjects(t, context.Background(), pool, "migration-0150")
	...
	pool.Close()
	if err := goose.DownTo(db, ".", 149); err == nil || !strings.Contains(err.Error(), "cannot roll back assessment relationship review while artifacts exist") {
