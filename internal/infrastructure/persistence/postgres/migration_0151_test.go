...
func TestMigration0151DefinesImmutableTenantSafeClosureReports(t *testing.T) {
	data, err := migrations.FS.ReadFile("0151_assessment_closure_reports.sql")
	if err != nil {
		t.Fatal(err)
	}
	sql := string(data)
...
func TestMigration0151EmptyRoundTrip(t *testing.T) {
	...
	if err := goose.DownTo(db, ".", 150); err != nil {
		t.Fatalf("rollback to 0150: %v", err)
	}
}

func TestMigration0151UpgradePaths(t *testing.T) {
	for _, from := range []int64{128, 140, 150} {
