...
func TestMigration0139DefinesBoundedProviderNeutralRLSSchema(t *testing.T) {
	contents, err := migrations.FS.ReadFile("0139_integration_framework.sql")
	if err != nil {
		t.Fatal(err)
	}
	sql := string(contents)
...
func TestPostgresIntegrationStoreAtomicityRLSCredentialsAndUpsert(t *testing.T) {
	...
	resolved, err := store.ResolveIntegrationCredential(actx, item.ID, "default")
	...
	if _, err := store.StartIntegrationOperation(actx, operation, jobKind, []byte(`{"operation_id":"`+operation.ID.String()+`"}`)); err != nil {
		t.Fatal(err)
	}
	conflicting := operation
	conflicting.ID, conflicting.JobID = shared.ID("operation-conflict-"+suffix), "job-conflict-"+suffix
	if _, err := store.StartIntegrationOperation(actx, conflicting, "integration.operation", []byte(`{}`)); !errors.Is(err, shared.ErrConflict) {
	...
	if _, err := store.StartIntegrationOperation(actx, cancelledOperation, jobKind, []byte(`{"operation_id":"`+cancelledOperation.ID.String()+`"}`)); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := store.StartIntegrationOperation(actx, deadLetterOperation, jobKind, []byte(`{"operation_id":"`+deadLetterOperation.ID.String()+`"}`)); err != nil {
		t.Fatal(err)
	}
	...
	if err := store.UpsertIntegrationExternalRuns(actx, []integration.ExternalRun{run}); err != nil {
		t.Fatal(err)
	}
