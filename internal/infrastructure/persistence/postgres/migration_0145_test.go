...
func TestMigration0145EmptyRoundTrip(t *testing.T) {
...
func TestMigration0145UpgradePaths(t *testing.T) {
	for _, from := range []int64{128, 140, 144} {
...
func TestMigration0145ClosureManifestGuards(t *testing.T) {
	ctx := context.Background()
	...
	tenantID := shared.ID("m145-tenant")
	cycleID, baseline, current := createAssessmentComparisonSnapshots(t, ctx, pool, tenantID, "m145")
	comparison := postgresQueuedComparison(t, tenantID, cycleID, "m145-comparison", baseline, current, 1, time.Now().UTC())
	...
	assessmentID := shared.ID("lineage-assessment-m145")
	manifestID := shared.ID("m145-manifest")
	...
	const rlsRole = "synapse_m145_rls"
	...
	if _, err := tx.Exec(ctx, `SELECT set_config('app.current_tenant','m145-other-tenant',true)`); err != nil {
		_ = tx.Rollback(ctx)
		pool.Close()
		t.Fatal(err)
	}
	...
	if err := goose.DownTo(db, ".", 144); err == nil || !strings.Contains(err.Error(), "cannot roll back assessment closure manifests") {
...
func TestAssessmentCycleScaleTargets(t *testing.T) {
	...
	tenantID := shared.ID("m145-scale-tenant")
	tx, err := pool.Begin(ctx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := tx.Exec(ctx, `INSERT INTO engagements (id,tenant_id,name,created_at,updated_at)
		SELECT 'm145-scale-eng-' || lpad(n::text,5,'0'),$1,'Scale assessment ' || n,
	...
	if _, err := tx.Exec(ctx, `INSERT INTO assessment_cycles
		...
		SELECT $1,'m145-scale-cycle-' || lpad(n::text,5,'0'),'Scale cycle ' || n,'standalone','open',
		       'm145-scale-eng-' || lpad(n::text,5,'0'),'m145-scale-eng-' || lpad(n::text,5,'0'),1,1,
	...
	if _, err := tx.Exec(ctx, `INSERT INTO assessment_cycle_members
		...
		SELECT $1,'m145-scale-cycle-' || lpad(n::text,5,'0'),'m145-scale-eng-' || lpad(n::text,5,'0'),'initial',0,1,
