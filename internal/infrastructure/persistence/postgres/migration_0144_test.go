...
func TestMigration0144EmptyRoundTrip(t *testing.T) {
	...
	if err := goose.DownTo(db, ".", 143); err != nil {
		t.Fatalf("empty rollback to 0142: %v", err)
	}
	var exists bool
...
func TestMigration0144UpgradeFixtureAndRollbackGuard(t *testing.T) {
	...
	if err := goose.UpTo(db, ".", 144); err != nil {
		t.Fatalf("upgrade fixture to 0143: %v", err)
	}
	pool, err = Connect(ctx, dsn)
	if err != nil {
		t.Fatal(err)
	...
	pool.Close()
	if err := goose.DownTo(db, ".", 143); err == nil || !strings.Contains(err.Error(), "cannot roll back finding lineage while lineage rows exist") {
