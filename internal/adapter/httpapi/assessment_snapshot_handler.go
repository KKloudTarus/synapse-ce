...
func (rt *Router) finalizeAssessmentSnapshot(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("ETag", snapshotETag(int64(snapshot.SnapshotNumber)))
	writeJSON(w, http.StatusCreated, finalizedAssessmentSnapshotResponse{Snapshot: newAssessmentSnapshotResponse(snapshot), DefaultVersion: int64(snapshot.SnapshotNumber)})
}
