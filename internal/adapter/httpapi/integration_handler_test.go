...
func newIntegrationHTTPRouter(t *testing.T) *Router {
	...
	service, err := integrationuc.NewService(
		memory.NewIntegrationStore(queue, cipher, clock), registry, memory.NewProjectRepository(),
		memory.MissingIntegrationAnalysisMatcher{}, ids, clock, integrationHTTPAudit{},
	)
	if err != nil {
		t.Fatal(err)
	}
