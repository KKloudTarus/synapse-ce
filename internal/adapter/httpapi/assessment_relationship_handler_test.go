...
func assessmentRelationshipHTTPRun(t *testing.T, runID, scope string, now time.Time) assessmentsnapshot.SelectedRun {
	t.Helper()
	target, err := scanrun.CanonicalTarget(scanrun.TargetInput{Kind: scanrun.TargetRepository, Raw: "https://example.com/repository.git", EvaluatedRevision: "0123456789abcdef0123456789abcdef01234567", SchemaVersion: 1})
	if err != nil {
		t.Fatal(err)
	}
	...
	lanes, err := scanrun.SealLanes([]scanrun.Lane{{
		...
		ResultRef: "result:" + runID, EvidenceRef: "evidence:" + runID, ResultSHA256: strings.Repeat("b", 64), ManifestSchemaVersion: 1,
	...
	manifest := sha256.Sum256([]byte("manifest:" + runID))
	return assessmentsnapshot.SelectedRun{ID: runID, ManifestHash: hex.EncodeToString(manifest[:]), Provenance: scanrun.ProvenanceNative, TerminalStatus: scanrun.StatusSucceeded, Trusted: true, Lanes: lanes}
}
