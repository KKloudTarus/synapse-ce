...
func TestSnapshotComplexityCoverageExcludesUnsupportedLanguagesFromRollup(t *testing.T) {
	input := BuildSnapshotInput{
		RuleCatalog: &mockResolver{},
		Inventory: NewInventory(nil,
			FileInventory{Path: "src/a.go", Language: "Go", CodeLines: 10},
			FileInventory{Path: "scripts/build.sh", Language: "Shell", CodeLines: 5},
		),
		Complexity: &ComplexityReport{
			Version: ComplexitySchemaVersion,
			Files: []ComplexityFileCoverage{
				{File: "src/a.go", Language: "Go", Supported: true, Parsed: true},
				{File: "scripts/build.sh", Language: "Shell", Supported: false},
			},
			Functions: []FunctionComplexity{{File: "src/a.go", Cyclomatic: 3, Cognitive: 2}},
		},
	}
	...
	unsupported := byPath["scripts/build.sh"]
	if unsupported.ComplexityAvailable || unsupported.ComplexityCoverage.EligibleFiles != 0 || unsupported.ComplexityCoverage.Reason != "unsupported_language" {
		t.Fatalf("unsupported file must be not applicable: %+v", unsupported)
	}
	root := byPath[""]
	if !root.ComplexityAvailable || root.ComplexityCoverage.EligibleFiles != 1 || root.ComplexityCoverage.MeasuredFiles != 1 || root.Counters.Cyclomatic != 3 {
		t.Fatalf("root must remain available over the supported subset: %+v", root)
	}
}

func TestSnapshotComplexityCoverageReasonIsDeterministic(t *testing.T) {
	input := BuildSnapshotInput{
		RuleCatalog: &mockResolver{},
		Inventory: NewInventory(nil,
			FileInventory{Path: "a.go", Language: "Go"},
			FileInventory{Path: "z.go", Language: "Go"},
		),
		Complexity: &ComplexityReport{
			Version: ComplexitySchemaVersion,
			Files: []ComplexityFileCoverage{
				{File: "z.go", Language: "Go", Supported: true, ParseError: true},
			},
		},
	}
	...
	for i := 0; i < 100; i++ {
		...
		if got := snapshot.Nodes[0].ComplexityCoverage.Reason; got != "complexity_coverage_missing" {
			t.Fatalf("iteration %d: root reason = %q, want deterministic first-child reason", i, got)
		}
	}
}
