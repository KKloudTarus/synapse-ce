...
// compareMaven orders two Maven versions using a faithful port of Apache Maven's ComparableVersion
// (mvnParse + mvnCmp in maven_comparable.go), so ranges are ordered exactly as Maven orders them,
// including the '-' sub-list NESTING a flat token comparison gets wrong (e.g. "1-1" < "1.1").
func compareMaven(a, b string) int {
	return mvnCmp(mvnParse(a), mvnParse(b))
}
