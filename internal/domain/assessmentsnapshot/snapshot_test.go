...
func TestCoverageDecisionReasonTable(t *testing.T) {
	...
	cases := []struct {
		...
		{"stage failed", base, func() scanrun.Lane {
			item := baseLane
			item.Stages = []scanrun.Stage{{Key: "scan", Status: scanrun.StageFailed}}
			return item
		}(), CoveragePartial, ReasonStageFailed},
		{"stage skipped", base, func() scanrun.Lane {
			item := baseLane
			item.Stages = []scanrun.Stage{{Key: "scan", Status: scanrun.StageSkipped}}
			return item
...
func TestComparabilityReasonTable(t *testing.T) {
	base := Dimension{
		...
		Versions: []Version{
			...
			{Kind: scanrun.VersionAdvisoryDatabase, Name: "advisories", Version: "1"},
	...
	cases := []struct {
		...
		{"advisory database changed", func(_ *Dimension, current *[]Dimension) {
			setVersion(&(*current)[0], scanrun.VersionAdvisoryDatabase, "2")
...
func selectedRun(id, producer string, findingKinds ...string) SelectedRun {
	...
	lane := scanrun.Lane{
		Key: id + "-lane", Producer: producer, TerminalStatus: scanrun.StatusSucceeded,
		Target:                    scanrun.TargetIdentity{Kind: scanrun.TargetRepository, SchemaVersion: 1, Canonical: "https://example.com/repo", EvaluatedRevision: "0123456789abcdef0123456789abcdef01234567"},
		...
		Versions: []scanrun.Version{{Kind: scanrun.VersionScanner, Name: "sca", Version: "1"}, {Kind: scanrun.VersionRulePack, Name: "rules", Version: "1"}},
		Stages:   []scanrun.Stage{{Key: "scan", Status: scanrun.StageSucceeded}},
	}
	sealed, err := scanrun.SealLanes([]scanrun.Lane{lane}, finished)
	if err != nil {
		panic(err)
	}
	return SelectedRun{ID: id, ManifestHash: "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc", Provenance: scanrun.ProvenanceNative, TerminalStatus: scanrun.StatusSucceeded, Trusted: true, Lanes: sealed}
}
