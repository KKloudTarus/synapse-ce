	...
	for _, run := range ordered {
		...
		for _, lane := range lanes {
			if err := scanrun.ValidateSealedLane(lane); err != nil {
				return nil, nil, "", fmt.Errorf("validate selected lane %s/%s: %w", run.ID, lane.Key, err)
			}
			reference.LaneReferences = append(reference.LaneReferences, LaneReference{LaneKey: lane.Key, ManifestHash: lane.ManifestHash})
...
func dimensionFrom(run SelectedRun, lane scanrun.Lane, findingKind string) Dimension {
	...
	for _, version := range lane.Versions {
		versions = append(versions, Version{Digest: version.Digest, Kind: version.Kind, Name: version.Name, Version: version.Version})
	}
	...
	return Dimension{
		...
		IncludedScope: sortedUnique(lane.IncludedScope), LaneKey: lane.Key, LaneManifestHash: lane.ManifestHash,
		Producer: lane.Producer, ReasonCode: reason, RunID: run.ID, State: state,
		Target:   Target{Canonical: lane.Target.Canonical, EvaluatedRevision: lane.Target.EvaluatedRevision, Kind: lane.Target.Kind, SchemaVersion: lane.Target.SchemaVersion},
		Versions: versions,
	}
}
