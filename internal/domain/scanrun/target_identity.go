...
func CanonicalizeRepositoryTarget(rawURL, rawRevision string) (TargetIdentity, error) {
	...
	if err != nil {
		return TargetIdentity{}, fmt.Errorf("%w: invalid repository hostname %q", shared.ErrValidation, hostname)
	}
	...
	if portStr != "" {
		hostHeader = asciiHost + ":" + portStr
	} else {
		hostHeader = asciiHost
	}
...
func CanonicalizeCloudResourceTarget(provider, accountOrProject, resourceID string) (TargetIdentity, error) {
	...
	canonical := fmt.Sprintf("%s/%s/%s", p, acct, res)
	return TargetIdentity{
