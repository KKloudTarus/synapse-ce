...
const (
	...
	maxLaneKeyLength   = 255
	maxProducerLength  = 255
...
func (s LaneStage) Validate() error {
	...
	if len(s.StageKey) > maxLaneKeyLength {
		return fmt.Errorf("%w: stage key exceeds %d bytes", shared.ErrValidation, maxLaneKeyLength)
	}
	if !s.Status.Valid() {
...
func (l Lane) Validate() error {
	...
	if len(l.LaneKey) > maxLaneKeyLength {
		return fmt.Errorf("%w: lane key exceeds %d bytes", shared.ErrValidation, maxLaneKeyLength)
	}
	if strings.TrimSpace(l.Producer) == "" {
		return fmt.Errorf("%w: producer is required", shared.ErrValidation)
	}
	if len(l.Producer) > maxProducerLength {
		return fmt.Errorf("%w: producer exceeds %d bytes", shared.ErrValidation, maxProducerLength)
	}
	...
	if len(l.ResultRef) > maxReferenceLength || len(l.EvidenceRef) > maxReferenceLength {
		return fmt.Errorf("%w: result/evidence reference exceeds %d bytes", shared.ErrValidation, maxReferenceLength)
	}
	if l.ResultSHA256 != "" && !hex64Regex.MatchString(l.ResultSHA256) {
		return fmt.Errorf("%w: result_sha256 must be 64 lowercase hex characters", shared.ErrValidation)
	}
	if l.ManifestHash != "" && !hex64Regex.MatchString(l.ManifestHash) {
		return fmt.Errorf("%w: manifest_hash must be 64 lowercase hex characters", shared.ErrValidation)
	}
...
func (r ScanRun) Validate() error {
	...
	if len(r.ID) > maxScanRunIDLength {
		return fmt.Errorf("%w: scan run ID exceeds %d bytes", shared.ErrValidation, maxScanRunIDLength)
	}
	...
	if r.ManifestHash != "" && !hex64Regex.MatchString(r.ManifestHash) {
		return fmt.Errorf("%w: manifest_hash must be 64 lowercase hex characters", shared.ErrValidation)
	}
...
//  4. All lanes succeeded and have non-empty authoritative finding kinds, valid
//     target revisions, a manifest hash, a sealed timestamp, and no failed or
...
func (r ScanRun) IsCompleteCoverage() bool {
	...
	for _, lane := range r.Lanes {
		if lane.TenantID != r.TenantID || lane.EngagementID != r.EngagementID || lane.ScanRunID != r.ID {
			return false
		}
		if lane.TerminalStatus != StatusSucceeded {
			return false
		}
		if lane.ManifestSchemaVersion != r.ManifestSchemaVersion {
			return false
		}
		...
		for _, stage := range lane.Stages {
			if stage.Status != StageSucceeded {
				return false
			}
		}
	}
...
// from PostgreSQL hashes exactly like the producer's in-memory representation.
type CanonicalLaneStage struct {
	StageKey   string      `json:"stage_key"`
	Status     StageStatus `json:"status"`
	ReasonCode string      `json:"reason_code,omitempty"`
...
const canonicalManifestTimeLayout = "2006-01-02T15:04:05.000000Z"
...
func ComputeManifestHash(lane Lane) (string, error) {
	...
	sort.SliceStable(versions, func(i, j int) bool {
		...
		return versions[i].Digest < versions[j].Digest
	})

	stages := make([]CanonicalLaneStage, len(lane.Stages))
	for i, stage := range lane.Stages {
		stages[i] = CanonicalLaneStage{
			StageKey:   stage.StageKey,
			Status:     stage.Status,
			ReasonCode: stage.ReasonCode,
			StartedAt:  canonicalManifestTime(stage.StartedAt),
		}
		if stage.FinishedAt != nil && !stage.FinishedAt.IsZero() {
			stages[i].FinishedAt = canonicalManifestTime(*stage.FinishedAt)
		}
	}
	sort.SliceStable(stages, func(i, j int) bool {
		if stages[i].StageKey != stages[j].StageKey {
			return stages[i].StageKey < stages[j].StageKey
		}
		if stages[i].Status != stages[j].Status {
			return stages[i].Status < stages[j].Status
		}
		if stages[i].ReasonCode != stages[j].ReasonCode {
			return stages[i].ReasonCode < stages[j].ReasonCode
		}
		if stages[i].StartedAt != stages[j].StartedAt {
			return stages[i].StartedAt < stages[j].StartedAt
		}
		return stages[i].FinishedAt < stages[j].FinishedAt
	})
...
func canonicalManifestTime(t time.Time) string {
	return t.UTC().Truncate(time.Microsecond).Format(canonicalManifestTimeLayout)
}
...
func ComputeRunManifestHash(lanes []Lane) (string, error) {
	...
	for i, lane := range copied {
		if i > 0 && copied[i-1].LaneKey == lane.LaneKey {
			return "", fmt.Errorf("%w: duplicate lane key %q", shared.ErrValidation, lane.LaneKey)
		}
		laneHash, err := ComputeManifestHash(lane)
		if err != nil {
			return "", fmt.Errorf("compute lane %q hash: %w", lane.LaneKey, err)
		}
		if stored := strings.TrimSpace(lane.ManifestHash); stored != "" && stored != laneHash {
			return "", fmt.Errorf("%w: lane %q manifest hash mismatch", shared.ErrValidation, lane.LaneKey)
		}
