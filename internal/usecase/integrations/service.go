...
func (service *Service) Create(ctx context.Context, input CreateInput) (integration.Integration, error) {
	...
	item := integration.Integration{
		...
		PollInterval: input.PollInterval, Version: 1, CreatedAt: now, UpdatedAt: now,
	}
...
func (service *Service) SetEnabled(ctx context.Context, tenantID, integrationID shared.ID, enabled bool, version int, actor string) (integration.Integration, error) {
	...
	if enabled {
		...
		operations, err := service.store.ListIntegrationOperations(tenantCtx, integrationID, defaultHistoryLimit)
		if err != nil {
			return integration.Integration{}, err
		}
		tested := false
		for _, operation := range operations {
			if operation.Type == integration.OperationTest && operation.State == integration.OperationSucceeded && operation.FinishedAt != nil && !operation.FinishedAt.Before(item.UpdatedAt) {
				tested = true
				break
			}
		}
		if !tested {
			return integration.Integration{}, fmt.Errorf("%w: test the connection successfully before enabling the integration", shared.ErrConflict)
		}
	}
...
func (service *Service) StartOperation(ctx context.Context, tenantID, integrationID shared.ID, operationType integration.OperationType, actor string) (integration.Operation, error) {
	...
	operation, err = service.store.StartIntegrationOperation(tenantCtx, operation, JobKind, payload)
	if err != nil {
		return integration.Operation{}, err
	}
	if err := service.record(tenantCtx, operation.Actor, "integration.operation_started", operation.ID, map[string]string{"integration_id": integrationID.String(), "operation": string(operationType), "provider": string(item.Provider)}); err != nil {
		return integration.Operation{}, err
	}
...
func (service *Service) HandleJob(ctx context.Context, jobID string, payload []byte) (err error) {
	...
	credentials, err := service.credentials(executionCtx, item.ID)
	if err != nil {
...
func (service *Service) executePoll(ctx context.Context, provider integration.Provider, operation integration.Operation, adapter integration.Adapter) error {
	...
	if err := service.store.UpsertIntegrationExternalRuns(ctx, allRuns); err != nil {
		return err
	}
...
func (service *Service) credentials(ctx context.Context, integrationID shared.ID) (integration.CredentialBundle, error) {
	plaintext, err := service.store.ResolveIntegrationCredential(ctx, integrationID, credentialIdentity)
	if err != nil {
		return nil, err
	}
