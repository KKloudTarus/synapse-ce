...
type AssessmentSnapshotBackfillStore interface {
	...
	SaveAssessmentSnapshotBackfillItem(ctx context.Context, item AssessmentSnapshotBackfillItem) (created bool, err error)
	AdvanceAssessmentSnapshotBackfillRun(ctx context.Context, tenantID, runID shared.ID, leaseOwner string, checkpoint shared.ID, now time.Time, leaseDuration time.Duration) (AssessmentSnapshotBackfillRun, error)
	FinishAssessmentSnapshotBackfillRun(ctx context.Context, tenantID, runID shared.ID, leaseOwner string, state AssessmentSnapshotBackfillState, now time.Time) (AssessmentSnapshotBackfillRun, error)
}
