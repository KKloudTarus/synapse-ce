...
type FindingLineageBackfillStore interface {
	...
	SaveFindingLineageBackfillItem(ctx context.Context, item FindingLineageBackfillItem) (created bool, err error)
	AdvanceFindingLineageBackfillRun(ctx context.Context, tenantID, runID shared.ID, leaseOwner string, checkpoint shared.ID, now time.Time, leaseDuration time.Duration) (FindingLineageBackfillRun, error)
	FinishFindingLineageBackfillRun(ctx context.Context, tenantID, runID shared.ID, leaseOwner string, state FindingLineageBackfillState, now time.Time) (FindingLineageBackfillRun, error)
}
