...
type AssessmentCycleBackfillStore interface {
	...
	SaveAssessmentCycleBackfillItem(ctx context.Context, item AssessmentCycleBackfillItem) (created bool, err error)
	AdvanceAssessmentCycleBackfillRun(ctx context.Context, tenantID, runID shared.ID, leaseOwner string, checkpoint shared.ID, now time.Time, leaseDuration time.Duration) (AssessmentCycleBackfillRun, error)
	FinishAssessmentCycleBackfillRun(ctx context.Context, tenantID, runID shared.ID, leaseOwner string, state AssessmentCycleBackfillState, now time.Time) (AssessmentCycleBackfillRun, error)
}
