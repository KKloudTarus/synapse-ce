...
func createComparisonSnapshot(t *testing.T, harness *comparisonHarness, number int, marker string, expectedVersion int64) *assessmentsnapshot.Snapshot {
	...
	target, err := scanrun.CanonicalTarget(scanrun.TargetInput{
		Kind: scanrun.TargetRepository, Raw: "https://example.com/repo.git", EvaluatedRevision: strings.Repeat(marker, 40), SchemaVersion: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
	harness.targetCanonical = target.Canonical
	run := ports.ScanRun{
		TenantID: harness.tenantID.String(), ID: fmt.Sprintf("run-%d", number), EngagementID: harness.assessmentID.String(), CreatedAt: started,
		...
		Lanes: []scanrun.Lane{{
			...
			ResultSHA256: strings.Repeat(marker, 64), ManifestSchemaVersion: 1,
