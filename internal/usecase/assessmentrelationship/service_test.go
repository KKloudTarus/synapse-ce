...
func relationshipSelectedRun(t *testing.T, runID, includedScope string, now time.Time) assessmentsnapshot.SelectedRun {
	...
	lanes, err := scanrun.SealLanes([]scanrun.Lane{{
		...
		ResultSHA256: strings.Repeat("b", 64), ManifestSchemaVersion: 1,
	...
	manifestSum := sha256.Sum256([]byte("manifest:" + runID))
	return assessmentsnapshot.SelectedRun{ID: runID, ManifestHash: hex.EncodeToString(manifestSum[:]), Provenance: scanrun.ProvenanceNative, TerminalStatus: scanrun.StatusSucceeded, Trusted: true, Lanes: lanes}
}
