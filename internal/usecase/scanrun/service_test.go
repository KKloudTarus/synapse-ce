...
func TestService_PostgresParity(t *testing.T) {
	dsn := isolatedScanRunServiceDSN(t)
	ctx := context.Background()
...
// isolatedScanRunServiceDSN keeps sealed provenance test data out of the
// shared integration database. This preserves migration rollback tests when
// packages run concurrently under go test ./....
func isolatedScanRunServiceDSN(t *testing.T) string {
	t.Helper()
	...
	databaseName := fmt.Sprintf("synapse_scanrun_service_%d", time.Now().UnixNano())
