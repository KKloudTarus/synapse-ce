...
func newClosureHarness(t *testing.T) closureHarness {
	...
	if err := api.SetClosureDependencies(cycles, snapshots, comparisons, queue, []byte("0123456789abcdef0123456789abcdef")); err != nil {
		t.Fatal(err)
	}
...
func closureSnapshot(t *testing.T, tenantID, cycleID, assessmentID, snapshotID shared.ID, runID string, now time.Time) *assessmentsnapshot.Snapshot {
	...
	lane := scanrun.Lane{
		Key: runID + "-lane", Producer: "sca", TerminalStatus: scanrun.StatusSucceeded,
	...
	lanes, err := scanrun.SealLanes([]scanrun.Lane{lane}, finished)
	if err != nil {
		t.Fatal(err)
	}
	snapshot, err := assessmentsnapshot.NewFinalized(tenantID, snapshotID, cycleID, assessmentID, assessmentsnapshot.Boundary{Kind: cycledom.BoundaryStandalone}, "request-"+snapshotID.String(), "scanner", now, []assessmentsnapshot.SelectedRun{{
		ID: runID, ManifestHash: "cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc", Provenance: scanrun.ProvenanceNative, TerminalStatus: scanrun.StatusSucceeded, Trusted: true, Lanes: lanes,
	}})
	if err != nil {
		t.Fatal(err)
	}
