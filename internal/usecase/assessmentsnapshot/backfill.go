...
func NewBackfillRunner(projector legacySnapshotProjector, source ports.AssessmentSnapshotBackfillSource, store ports.AssessmentSnapshotBackfillStore, runs ports.ScanRunStore, results ports.ScanResultStore, ids ports.IDGenerator, clock ports.Clock, audit ports.AuditLogger, observer AssessmentSnapshotBackfillObserver) (*BackfillRunner, error) {
...
func (runner *BackfillRunner) Run(ctx context.Context, request BackfillRequest) (ports.AssessmentSnapshotBackfillRun, error) {
	...
	run, resumed, err := runner.store.AcquireAssessmentSnapshotBackfillRun(ctx, ports.AssessmentSnapshotBackfillAcquireRequest{
		Run: ports.AssessmentSnapshotBackfillRun{
			TenantID: tenantID, ID: runner.ids.NewID(), SchemaVersion: AssessmentSnapshotBackfillSchemaVersion,
			DryRun: request.DryRun, BatchSize: batchSize, SnapshotAt: now, State: ports.AssessmentSnapshotBackfillRunning,
			LeaseOwner: leaseOwner, LeaseExpiresAt: now.Add(leaseDuration), CreatedBy: actor, CreatedAt: now, UpdatedAt: now,
		},
	...
	for {
		...
		for _, assessment := range assessments {
			...
			item := runner.processItem(ctx, run, assessment.ID, actor)
			created, err := runner.store.SaveAssessmentSnapshotBackfillItem(ctx, item)
			if err != nil {
				return runner.finish(ctx, run, leaseOwner, snapshotBackfillTerminalState(err), actor, err)
			}
		...
		run, err = runner.store.AdvanceAssessmentSnapshotBackfillRun(ctx, tenantID, run.ID, leaseOwner, checkpoint, runner.clock.Now().UTC(), leaseDuration)
		if err != nil {
			return runner.finish(ctx, run, leaseOwner, snapshotBackfillTerminalState(err), actor, err)
		}
...
func (runner *BackfillRunner) loadProjectionSource(ctx context.Context, run ports.AssessmentSnapshotBackfillRun, assessmentID shared.ID) (projectionSource, error) {
	...
	runs, err := runner.runs.List(ctx, run.TenantID, assessmentID)
	if err != nil {
		return projectionSource{}, err
	}
	eligible := make([]ports.ScanRun, 0, len(runs))
	...
	for _, candidate := range eligible {
		if candidate.Provenance != scanrun.ProvenanceNative || candidate.ValidateSealed() != nil {
			continue
		}
...
func validateProjectionRun(run ports.ScanRun, tenantID, assessmentID shared.ID) error {
	if strings.TrimSpace(run.ID) == "" || run.TenantID != tenantID.String() || run.EngagementID != assessmentID.String() || run.CreatedAt.IsZero() {
...
func (runner *BackfillRunner) finish(ctx context.Context, run ports.AssessmentSnapshotBackfillRun, leaseOwner string, state ports.AssessmentSnapshotBackfillState, actor string, cause error) (ports.AssessmentSnapshotBackfillRun, error) {
	...
	defer cancel()
	finished, err := runner.store.FinishAssessmentSnapshotBackfillRun(finishCtx, run.TenantID, run.ID, leaseOwner, state, runner.clock.Now().UTC())
	if err != nil {
		if cause != nil {
