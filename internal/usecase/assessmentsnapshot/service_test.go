...
func TestFinalizeRejectsTerminalAssessmentAndBuildingRun(t *testing.T) {
	...
	other := newHarness(t)
	building := ports.ScanRun{TenantID: "tenant", ID: "building", EngagementID: "assessment", CreatedAt: other.clock.now, Provenance: scanrun.ProvenanceNative, TerminalStatus: scanrun.StatusBuilding}
	if err := other.runs.Begin(context.Background(), building); err != nil {
		t.Fatal(err)
	}
...
func (h *harness) saveRun(t *testing.T, run ports.ScanRun) {
	t.Helper()
	begin := run
	...
	if err := h.runs.Begin(context.Background(), begin); err != nil {
		t.Fatal(err)
	}
	if err := h.runs.Seal(context.Background(), run); err != nil {
		t.Fatal(err)
	}
}

func nativeRun(t *testing.T, id, assessmentID, revision string) ports.ScanRun {
	t.Helper()
	started := time.Date(2026, 8, 31, 11, 0, 0, 0, time.UTC)
	finished := started.Add(time.Minute)
	target, err := scanrun.CanonicalTarget(scanrun.TargetInput{Kind: scanrun.TargetRepository, Raw: "https://example.com/repo.git", EvaluatedRevision: revision, SchemaVersion: 1})
	if err != nil {
		t.Fatal(err)
	}
	run := ports.ScanRun{
		TenantID: "tenant", ID: id, EngagementID: assessmentID, CreatedAt: started,
