...
type staticBackfillRunStore struct{ runs []ports.ScanRun }

func (store *staticBackfillRunStore) Begin(context.Context, ports.ScanRun) error { return nil }
func (store *staticBackfillRunStore) Seal(context.Context, ports.ScanRun) error  { return nil }
func (store *staticBackfillRunStore) List(_ context.Context, tenantID, assessmentID shared.ID) ([]ports.ScanRun, error) {
	var out []ports.ScanRun
	for _, run := range store.runs {
		if run.TenantID == tenantID.String() && run.EngagementID == assessmentID.String() {
			out = append(out, run)
		}
	}
	return out, nil
}
func (store *staticBackfillRunStore) Get(_ context.Context, tenantID shared.ID, runID string) (ports.ScanRun, error) {
	for _, run := range store.runs {
		if run.TenantID == tenantID.String() && run.ID == runID {
			return run, nil
		}
	}
	return ports.ScanRun{}, shared.ErrNotFound
}
...
func TestSnapshotBackfillProjectsUnknownCoverageAndHashVersionedEvidence(t *testing.T) {
	...
	harness.runs.runs = []ports.ScanRun{run}
...
func TestSnapshotBackfillProjectsLegacyRunWithoutInventedDimensions(t *testing.T) {
	legacy := ports.ScanRun{
		...
		CreatedAt: time.Date(2026, 8, 31, 9, 0, 0, 0, time.UTC), Provenance: scanrun.ProvenanceLegacy, TerminalStatus: scanrun.StatusUnknown,
		Manifest: ports.ScanManifest{CorrelationVersion: 1}, FindingKeys: []string{"legacy-finding"},
	}
	harness := newSnapshotBackfillHarness(t, []ports.ScanRun{legacy})
...
func TestSnapshotBackfillLeaseResumeDryRunAndRedactedRetry(t *testing.T) {
	harness := newSnapshotBackfillHarness(t, []ports.ScanRun{nativeRun(t, "sealed-run", "assessment", "0123456789abcdef0123456789abcdef01234567")})
	...
	crashed, _, err := harness.store.AcquireAssessmentSnapshotBackfillRun(context.Background(), ports.AssessmentSnapshotBackfillAcquireRequest{Run: ports.AssessmentSnapshotBackfillRun{
		...
		SnapshotAt: harness.clock.Now(), State: ports.AssessmentSnapshotBackfillRunning, LeaseOwner: "dead", LeaseExpiresAt: harness.clock.Now().Add(lease),
	...
	retryHarness := newSnapshotBackfillHarness(t, []ports.ScanRun{nativeRun(t, "retry-run", "assessment", "0123456789abcdef0123456789abcdef01234567")})
	...
	cancelHarness := newSnapshotBackfillHarness(t, []ports.ScanRun{nativeRun(t, "cancel-run", "assessment", "0123456789abcdef0123456789abcdef01234567")})
...
func newSnapshotBackfillHarness(t *testing.T, runs []ports.ScanRun) *snapshotBackfillHarness {
	t.Helper()
