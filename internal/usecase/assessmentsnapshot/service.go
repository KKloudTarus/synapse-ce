...
func NewService(snapshots ports.AssessmentSnapshotRepository, cycles ports.AssessmentCycleRepository, engagements ports.EngagementRepository, runs ports.ScanRunStore, tx ports.TenantTransactionRunner, ids ports.IDGenerator, clock ports.Clock, audit ports.AuditLogger) (*Service, error) {
...
func (service *Service) Finalize(ctx context.Context, input FinalizeInput) (*domain.Snapshot, bool, error) {
	...
	err = service.tx.Run(ctx, tenantID, func(txCtx context.Context) error {
		...
		for _, selection := range selectedRuns {
			...
			run, err := service.runs.Get(txCtx, tenantID, runID)
			if err != nil {
				return err
			}
			if run.EngagementID != input.AssessmentID.String() {
...
func trustedSelectedRun(run ports.ScanRun, laneKeys []string) (domain.SelectedRun, error) {
...
func legacyRunHash(run ports.ScanRun) string {
	findingKeys := append([]string(nil), run.FindingKeys...)
	...
	}{run.CreatedAt.UTC().Format("2006-01-02T15:04:05.999999999Z07:00"), run.EngagementID, findingKeys, run.ID, run.Manifest})
...
func (service *Service) expandRunSelections(ctx context.Context, tenantID, assessmentID shared.ID, selections []RunSelection) ([]RunSelection, error) {
	for index := range selections {
		...
		run, err := service.runs.Get(ctx, tenantID, selections[index].RunID)
		if err != nil {
			return nil, err
		}
		if run.EngagementID != assessmentID.String() {
