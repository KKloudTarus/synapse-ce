...
func (c *Catalog) Tools() []agent.ToolSchema {
	...
	if c.jproposer != nil {
		...
		schemas = append(schemas, agent.ToolSchema{
			...
			Parameters:  json.RawMessage(`{"type":"object","properties":{"incident_id":{"type":"string","description":"the incident this hypothesis is about; read it first with get_incident_detail"},"tactic":{"type":"string","description":"one of: lateral_movement | data_exfiltration | privilege_escalation | credential_access | persistence | command_and_control | defense_evasion | execution | benign"},"confidence":{"type":"integer","description":"0..100 confidence in the hypothesis"},"drivers":{"type":"array","minItems":1,"items":{"type":"string"},"description":"the signal TOKENS that support it, e.g. new_exec_paths, network_fanout_spike, privilege_events (lowercase tokens, no spaces, no prose). At least one is required: read the incident with get_incident_detail and cite what you saw."}},"required":["incident_id","tactic","confidence","drivers"],"additionalProperties":false}`),
		})
	}
	if c.incidents != nil {
		schemas = append(schemas, agent.ToolSchema{
